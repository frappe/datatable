  ...
  _createClass(ReGrid, [{
    ...
    value: function bindEditCell() {
      ...
      this.bodyScrollable.on('dblclick', '.data-table-col', function () {
        ...
        var cell = self.getCell(rowIndex, colIndex);

        $editCell.find('input').val(cell.content);
        // $editPopup.html(getEditCellHTML(cellValue.content));

        var width = $cell.find('.content').width() + 1;
        var height = $cell.find('.content').height() + 1;

        var selector = '.data-table .body-scrollable [data-row-index="' + rowIndex + '"][data-row-index="' + rowIndex + '"] .edit-cell';

        self.setStyle(selector, { width: width, height: height });
        $editCell.show();
      ...
      (0, _jQuery2.default)(document.body).on('click', function (e) {
        if ((0, _jQuery2.default)(e.target).is('.edit-cell, .edit-cell *')) return;
      ...
      if (styles.match(rulePattern)) {
        var _loop = function _loop(property) {
          ...
          styles = styles.replace(rulePattern, function (match, propertyStr) {
            ...
            var replacer = rule + ' {' + propertyStr + self.getCSSString(_styleMap) + '}';
    ...
    key: 'getCSSString',
    value: function getCSSString(styleMap) {
      var style = '';
